...
function SetTitleFromLecturesSQL(id_list_name, index){
    db.transaction(function (lecture_table) {
        lecture_table.executeSql('select * from LOGS', [], function (lecture_table, results) {
            document.querySelector("#"+ id_list_name + index).innerHTML = results.rows.item(index).title;
        })
    })
}
...
function SetHrefFromLecturesSQL(id_list_name, index){
    db.transaction(function (lecture_table) {
        lecture_table.executeSql('select * from LOGS', [], function (lecture_table, results) {
            document.querySelector("#" + id_list_name + index).setAttribute("href",
...
function SetDateFromLecturesSQL(id_list_name, index){
    db.transaction(function (lecture_table) {
        lecture_table.executeSql('select * from LOGS', [], function (lecture_table, results) {
            document.querySelector("#" + id_list_name + index).innerHTML = results.rows.item(index).date;
        })
    })
}
